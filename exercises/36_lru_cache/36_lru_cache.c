...
static void lru_put(LRUCache* c, int key, int value) {
    ...
    if (!he) { /* rollback node */ free(node); return; }
    he->key = key;
    he->node = node;
    ...
    if (c->size > c->capacity) {
        ...
        if (tail) {
            /* remove from hash */
            unsigned idx2 = hash_int(tail->key) % c->bucket_count;
            HashEntry** pp2 = &c->buckets[idx2];
            while (*pp2) {
                if ((*pp2)->key == tail->key) {
                    HashEntry* del = *pp2;
                    *pp2 = del->next;
